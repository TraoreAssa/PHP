...
if(isset($_POST['ajouter_panier']))
{
	...
	ajouterProduitDansPanier($produit['titre'], $_POST['id_produit'], $_POST['quantite'], $produit['prix'],$produit['photo'],$produit['stock']);
}
...
echo '<tr><th>Titre</th><th>Produit</th><th>Quantité</th><th>PrixHT</th><th>PrixTTC</th></tr>';
...
if(empty($_SESSION['panier']['id_produit'])){
	...
	echo '<tr><td colspan="5">Votre panier est vide</td></tr>';
		...
		for($i = 0; $i < count($_SESSION['panier']['id_produit']); $i++){
			...
			echo'<td><form method="post"><select name="qtevariant" onchange="change()">';
			
			
			for($j=0; $j < $_SESSION['panier']['stock'][$i]; $j++){
			...
			echo '<td>' . $_SESSION['panier']['prix'][$i]*1.195. '</td>';
		...
		// recalcul des montant en fonction des qtés rentrées




		echo "<tr><th colspan='3'>Total</th><td>" . montantTotal() . " euros</td><td>" . round(montantTotal()*1.195,2) . " euros</td></tr>";
...
echo "<hr>session panier.php : <br>"; debug($_SESSION['panier']);?>
...
<script type="text/javascript" > function change(){
	...
	for(i=0;i<querySelectorAll("option").lenght;i++){

		if(querySelectorAll("option")[i].selected){
			resultat=i;
		}
	}
		querySelectorAll("option")[resultat].innerHTML*
}
	</script>
